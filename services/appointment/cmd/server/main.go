...
func main() {
	...
	if err != nil {
		panic(fmt.Sprintf("开启appointment grpc端口监听失败:%s", err.Error()))
	}
	server, err := rpc.NewAppointmentServe()
	if err != nil {
		panic(fmt.Sprintf("实例appointment server失败:%s", err.Error()))
	}

	s := grpc.NewServer()
	proto.RegisterAppointmentServerServer(s, server)
	reflection.Register(s)

	go heart(server)

	if err := s.Serve(listen); nil != err {
		panic(fmt.Sprintf("开启appointment grpc失败:%s", err.Error()))
	}
}
...
func heart(s *rpc.Server) {
	for true {
		if err := s.Heart(context.Background()); nil != err {
			util.Logger.Error(fmt.Sprintf("appointment 服务心跳失败:%s", err.Error()))
		}
